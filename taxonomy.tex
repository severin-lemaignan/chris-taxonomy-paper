...
\usepackage{natbib} % for citations like (Tomasello, 2009) instead of [1]
...
\item[Goal]: We refer to goal as a (mental) representation of a desired state
...
\item[Intention]: An intention is the plan of action the agent chooses and
...
\item[Outcome]: The outcome or result of an action refers to the actual and
...
outcome (the observable state of the environment) do not match.
	...
	\item[Success]: The action changed the state of the environment so that
	outcome and goal now correspond.  This usually leads to feelings of
	satisfaction by the agent.

	\item[Failed attempt]: The action did not result in a change of state of the
	...
	\item[Accident]: The action resulted in a change of the environment, but
...
\item[Parallel actions]: Both agents perform the same kind of action.

	\begin{itemize}

	\item{\it Example 1}: We hold the two ends of a heavy board and lift it together.

	\item{\it Example 2}: We pull two parallel ropes to move a heavy objects towards us.

	\item{\it Example 3}: We both play on our drums.

	\end{itemize}

\item[Complementary actions]: Both agents perform different but interrelated
actions.

	\begin{itemize}

	\item{\it Example 1}: I hold the frame steady so that you can nail it to the wall.

	\item{\it Example 2}: I hold and turn the table top so that you can attach the four legs
	one by one.
